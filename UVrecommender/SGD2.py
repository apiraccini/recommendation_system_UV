...
def stochastic_gradient_descent_mat2(M, d, eta=0.0001, onlyUV = False, perturb = False, nperturb = 3, method="n", init="mean"):
    n = np.shape(M)[0]  
    p = np.shape(M)[1]
    ...
    if perturb == False:    
        ...
        while not CONVERGED and num_iter < MAX_ITER:
            ...
            s = random.sample(S,int(nobs*0.1))
            for i in s:
...
def main():
    ...
    if perturb == False:
        (U, V, rmse, num_iter, perc_corretti) = stochastic_gradient_descent_mat2(M, d, eta)	
        ...
        (U, V ,rmse, avg_num_iter, perc_corretti) = stochastic_gradient_descent_mat2(M, d, eta, onlyUV = False, perturb = perturb, nperturb = nperturb, init = init, method = method)
    ...
    if perturb == False:
        ...
        (U, V,rmse, avg_num_iter, percorretti) = stochastic_gradient_descent_mat2(M, d, eta, onlyUV = False, perturb = perturb, nperturb = nperturb, init = init, method = method)
